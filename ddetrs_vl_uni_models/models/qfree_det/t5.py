...
try:
    ...
    _has_peft = False
    print('⚠️ peft not found, using fallback dummy Lora configuration.')

    class LoraConfig:
        def __init__(self, *args, **kwargs):
            raise RuntimeError('peft is required for LoraConfig. install `peft` or disable T5/adapter mode.')

    def get_peft_model(model, config):
        raise RuntimeError('peft is required for get_peft_model. install `peft` or disable T5/adapter mode.')
