...
from ._deformable_detr.backbone import Joiner
from ._deformable_detr.deformable_detr import DeformableDETR, SetCriterion
from ._deformable_detr.matcher import HungarianMatcher, HungarianMatcherVL
from ._deformable_detr.position_encoding import PositionEmbeddingSine
from ._deformable_detr.deformable_transformer import DeformableTransformer

from .segmentation_condInst_new_encodfpn_3 import DETRsegmVLUni, segmentation_postprocess
...
from .qfree_det.qfreeDet import build_qfree_det
...
from .qfree_det.utils import MLP
...
class DDETRSALUni(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # else:
        #     raise ValueError(f"Unsupported model: {MODEL_DET}")
        ...
        clip_name = getattr(cfg.MODEL, "CLIP", None)
        clip_name = clip_name.NAME if clip_name is not None else "ViT-L/14@336px"
        self.clip_model, self.preprocess_clip = clip.load(clip_name)
        self.clip_model.to(self.device)
        self.lang_dim = cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM
        self.clip_embed_dim = getattr(cfg.MODEL.CLIP, "EMBED_DIM", self.lang_dim)

        for p in self.clip_model.parameters():
                p.requires_grad_(False)

        self.project_tt = nn.Linear(self.lang_dim, self.lang_dim)
        self.project_tt.weight.data = torch.zeros(self.lang_dim, self.lang_dim)
        self.project_tt.bias.data = torch.zeros(self.lang_dim)
        self.mapping_text = MLP(self.lang_dim, 256, self.lang_dim, 3)
        if self.clip_embed_dim == self.lang_dim:
            self.clip_to_lang_proj = nn.Identity()
        else:
            self.clip_to_lang_proj = nn.Linear(self.clip_embed_dim, self.lang_dim)
        ...
        self.clip_embed_dim = self.lvis_embed.shape[0]
        if isinstance(self.clip_to_lang_proj, nn.Linear) and self.clip_to_lang_proj.in_features != self.clip_embed_dim:
            self.clip_to_lang_proj = nn.Linear(self.clip_embed_dim, self.lang_dim)
    ...
    def language_feature_forward(self, targets):
        ...
        clip_object_descriptions_features = torch.zeros(len(targets), max_len, self.lang_dim).to(self.device)
        ...
        for ii, target in enumerate(targets):
            ...
            text_features = self.clip_to_lang_proj(text_features)
