...
class MLP_Projection(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.linear_gating = nn.Linear(input_dim, output_dim, bias=False)
...
class VL_Align_Ours(nn.Module):
    def __init__(self, cfg):
        ...
        self.dot_product_projection_image = nn.Linear(cfg.MODEL.DDETRS.HIDDEN_DIM, lang_dim, bias=True) # nn.Identity()
        self.dot_product_projection_back = nn.Linear(cfg.MODEL.DDETRS.HIDDEN_DIM, lang_dim, bias=True) # nn.Identity()
        self.back_dec = nn.Linear(lang_dim * 2, lang_dim)

        self.text_embedding_low = nn.Linear(lang_dim, 256)
        self.img_256 = nn.Linear(256, 256)

        self.fusion = MLP_Projection(256, 2048, lang_dim, 3)
        self.fusion_enc = MLP_Projection(256, 2048, lang_dim, 3)
        self.fusion_back = MLP_Projection(256, 2048, lang_dim, 3)

        self.norm_fuse = nn.LayerNorm(lang_dim)
        ...
        self.bias_lang = nn.Parameter(torch.zeros(lang_dim), requires_grad=True) # (lang_dim,)
